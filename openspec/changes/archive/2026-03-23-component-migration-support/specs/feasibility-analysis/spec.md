...
The system SHALL validate a migration plan by simulating each step in order. Before applying a step, the system SHALL check that all dependency preconditions for the target component's transition are satisfied by the current simulated state of all components. For structural steps, the system SHALL additionally apply retirement constraint checks (see ADDED Requirements). If any precondition is not met, the step is marked as a violation.
