...
The system SHALL parse `graph` and `flowchart` Mermaid syntax using the Mermaid internal AST parser rather than regex, correctly handling all arrow types (`-->`, `--->`, `==>`, `-.->`, `--o`, `--x`), edge labels, and node shapes defined in the Mermaid grammar.
...
The system SHALL parse `sequenceDiagram` Mermaid syntax using the Mermaid internal AST parser, correctly handling `participant`, `actor`, all arrow variants (`->>`, `-->>`, `->`, `-->`, `-x`, `--x`, `-)`, `--)`), and `%%` comments.
...
The system SHALL catch parse errors from the Mermaid parser and surface a human-readable error message in the import dialog, without creating a partial diagram.
